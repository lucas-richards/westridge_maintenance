...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
                ...
                <div style=" border-bottom: 1px solid #ddd;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div style="display: flex; align-items: baseline;">
                            ...
                            <p class="truncate" style="min-width: 250px; margin-right: 8px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{data.workorder.title}</p>
                        </div>
                        <div>
        ...
        for workorder in workorders:
            ...
            if last_record and last_record.due_date:
                # day of event
                if workorder.notification == 'day of event' and (last_record.due_date - timezone.now()).days <= 0:
                    self.send_notification_email(workorder, last_record)
                elif workorder.notification == 'day before' and (last_record.due_date - timezone.now()).days <= 1:
                    self.send_notification_email(workorder, last_record)
    ...
    def send_notification_email(self, workorder, record):
        if workorder.assigned_to and workorder.assigned_to.email:
            ...
            subject = f'Westridge Labs - Work Order #{workorder.id} - {workorder.title}'
            message = f'''
            <div style="padding: 16px; border: 1px solid #ddd; border-radius: 4px;cursor:pointer" >
                <h3 style="color: #007bff;">Reminder: Work Order <a href="http://10.1.1.19/maintenance/workorders/?idwo={workorder.id}" target="_blank">#{workorder.id}</a> Due Soon</h3>
